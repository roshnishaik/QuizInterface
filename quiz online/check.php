...
include("config.php");
include("config1.php");
$sql1='SELECT * from quizusers WHERE username="'.$user.'"';
$retval1 = mysql_query( $sql1, $link );
$row1 = mysql_fetch_array($retval1);
$i=$row1['random'];

/*checking for 1 */
$sql = 'SELECT * FROM quizquestions WHERE id="'.$i.'"';
...
if($row1['marks']<=$count)
{
$query = "UPDATE quizusers SET marks='".$count."' WHERE  username='".$user."'";
...
$query= "UPDATE quizusers SET random='0' WHERE username='".$user."'";
$retval = mysql_query( $query, $link1 );

      $To =$user; // sender
      $From="quiz.quest2k14@gmail.com";
      $subject ="COMPLETION OF TEST";
      $message = "THANK YOU FOR COMPLETING THE KVIZO ONLINE ROUND 1
      
YOUR ANSWERS ARE STORED AND WILL BE EVALUATED SOON.

A MAIL WILL BE SENT TO YOU IF YOU ARE SELECTED FOR KVIZO SECOND ROUND.
...
window.location="feed.php?user=<?php echo $user;?> & flag=1" 
//--> 
</script>
</head>
</html>